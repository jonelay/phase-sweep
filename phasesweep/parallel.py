...
Pattern adapted from stillspin (github.com/jonelay/stillspin).
"""
...
def execute_generic(
    ...
    def _harvest(future: Any, idx: int) -> dict[str, Any]:
        try:
    ...
    try:
        ...
        try:
            for future in as_completed(future_to_idx, timeout=_FUTURE_TIMEOUT_S):
                pending.discard(future)
                _record(_harvest(future, future_to_idx[future]))
        except TimeoutError:
            # Stragglers past the pool timeout become TIMEOUT results
            # instead of blocking the pool forever; anything that finished
            # before the deadline is still harvested
            for future in pending:
                ...
                if future.done():
                    ...
                    _record(_failure_result(
                        jobs[idx], "TIMEOUT", float(_FUTURE_TIMEOUT_S),
                        f"Exceeded {_FUTURE_TIMEOUT_S}s pool timeout",
                    ))
