...
from phasesweep.fem_wedge import (
    compute_k_end,
    slab_analytic,
    solve_slab,
...
from phasesweep.geometry import default_inrunner, outrunner


def _actuator_outrunner():
    return outrunner(
...
def test_wedge_outrunner_matches_zhu_howe():
    r = solve_wedge_2d_equiv(_actuator_outrunner(), n_p=6, B_rem=1.45)
    assert r.rel_error < 5e-3
...
def test_endeffect_profile_and_k_end():
    ...
    r = solve_wedge_endeffect(_actuator_outrunner(), **_FAST_END)
    n = len(r.b3)
...
def test_endeffect_top_bc_runs():
    r = solve_wedge_endeffect(_actuator_outrunner(), top_bc="dirichlet", **_FAST_END)
    # Grounding the top of the air box barely moves k_end (0.9412 vs 0.9428).
    assert r.k_end == pytest.approx(_K_END_FAST, abs=_K_END_BAND)


def test_endeffect_tight_gap_solves_at_defaults():
    # 0.262 mm gap (aluminum rotor): local gap refinement keeps the bulk coarse so
...
def test_wedge_picard_nonconvergence_raises(monkeypatch):
    ...
    import phasesweep.fem_wedge as fw
    ...
    with pytest.raises(RuntimeError, match="did not converge"):
        solve_wedge_endeffect(_actuator_outrunner(), nonlinear=True,
                              **_FAST_END)
