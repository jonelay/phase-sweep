...
Acceptance targets:
1. Recover effective B_rem 1.139 T on the 14 mm outrunner back-EMF data — the
...
OUTRUNNER_TOML = REPO_ROOT / "motors/outrunner_14mm_steel.toml"
BACKEMF_JSON = REPO_ROOT / "data/outrunner_14mm_steel/backemf_measured.json"
PSI_F_MEAS = 0.000268  # Wb, 14 mm outrunner back-EMF speed sweep
...
def outrunner_motor():
    return load_motor(OUTRUNNER_TOML)


@pytest.fixture(scope="module")
def outrunner_backemf(outrunner_motor):
    """The committed import-format dataset from the speed sweep."""
    data = MeasuredResult.from_dict(json.loads(BACKEMF_JSON.read_text()))
    validate_measured(data)
    return measured_run_result(data, outrunner_motor, BACKEMF_JSON.stem)
...
class TestRecoverEffectiveBrem:
    ...
    def result(self, outrunner_motor, outrunner_backemf):
        return calibrate(
            outrunner_motor, [outrunner_backemf],
    ...
    def test_config_ids_differ_and_recorded(self, result, outrunner_motor):
        rec = result.record
        assert rec.source_config_id == outrunner_motor.config_id
...
class TestCreatorBCore:
    ...
    @pytest.fixture(scope="class")
    def dataset(self):
        path = _IRON_LOSS_JSON
        return MeasuredResult.from_dict(json.loads(path.read_text())), path.stem
...
class TestSyntheticRoundTrip:
    ...
    def test_recovers_perturbed_truth(self, outrunner_motor):
        """Calibrating against self-generated data recovers the known delta."""
        truth = perturb_motor(outrunner_motor, "B_rem", -0.10)
...
class TestMultiDatasetRows:
    ...
    def _synthetic(self, outrunner_motor, delta, dataset_id):
        truth = perturb_motor(outrunner_motor, "B_rem", delta)
        assert truth is not None
    ...
    def test_both_datasets_enter_the_fit(self, outrunner_motor):
        low = self._synthetic(outrunner_motor, -0.10, "sweep_low")
        high = self._synthetic(outrunner_motor, -0.06, "sweep_high")
        result = calibrate(
            outrunner_motor, [low, high],
            params=["B_rem"], quantities=["backemf_fundamental"],
        )
        rec = result.record
    ...
    def test_duplicate_dataset_ids_deduped(self, outrunner_motor):
        a = self._synthetic(outrunner_motor, -0.10, "same_id")
        b = self._synthetic(outrunner_motor, -0.06, "same_id")
        result = calibrate(
            outrunner_motor, [a, b],
            params=["B_rem"], quantities=["backemf_fundamental"],
        )
...
class TestGuards:
    ...
    def test_circular_fit_refused(self, outrunner_motor):
        """Fitting a param against the dataset it was derived from is an echo."""
        motor = replace(outrunner_motor, psi_f=PSI_F_MEAS)
    ...
    def test_circular_extends_to_interderivable_pair(self, outrunner_motor):
        """psi_f derived from the dataset blocks fitting B_rem too."""
        backemf = PSI_F_MEAS * outrunner_motor.drive.W_REF * outrunner_motor.n_p
        meas = _measured(
            outrunner_motor, {"backemf_fundamental": backemf},
            derived_params=("psi_f",),
        )
        with pytest.raises(ValueError, match="circular fit refused"):
            calibrate(outrunner_motor, [meas], params=["B_rem"],
                      quantities=["backemf_fundamental"])
    ...
    def test_underdetermined_refused(self, outrunner_motor, outrunner_backemf):
        with pytest.raises(ValueError, match="under-determined"):
            calibrate(outrunner_motor, [outrunner_backemf],
                      params=["B_rem", "gap"],
                      quantities=["backemf_fundamental"])

    def test_psi_f_and_brem_together_refused(self, outrunner_motor, outrunner_backemf):
        with pytest.raises(ValueError, match="interderivable"):
            calibrate(outrunner_motor, [outrunner_backemf],
                      params=["psi_f", "B_rem"],
                      quantities=["backemf_fundamental"])

    def test_unknown_param_refused(self, outrunner_motor, outrunner_backemf):
        with pytest.raises(ValueError, match="unknown fittable param"):
            calibrate(outrunner_motor, [outrunner_backemf],
                      params=["n_p"], quantities=["backemf_fundamental"])

    def test_unset_param_refused(self, outrunner_backemf, outrunner_motor):
        motor = replace(outrunner_motor, L_d=None, L_q=None)
        with pytest.raises(ValueError, match="not set"):
            calibrate(motor, [outrunner_backemf],
    ...
    def test_bound_for_unfitted_param_refused(self, outrunner_motor, outrunner_backemf):
        with pytest.raises(ValueError, match="matches no fitted param"):
            calibrate(outrunner_motor, [outrunner_backemf],
                      params=["B_rem"], quantities=["backemf_fundamental"],
                      bounds={"k_w": (-0.1, 0.1)})

    def test_inverted_bound_refused(self, outrunner_motor, outrunner_backemf):
        with pytest.raises(ValueError, match="lower must be < upper"):
            calibrate(outrunner_motor, [outrunner_backemf],
                      params=["B_rem"], quantities=["backemf_fundamental"],
                      bounds={"B_rem": (0.3, -0.3)})

    def test_bounds_excluding_zero_clamp_x0(self, outrunner_motor, outrunner_backemf):
    ...
    def test_auto_models_never_pick_thermal_duty(self, outrunner_motor):
        """thermal_duty needs a duty_profile the calibration loop's
        RunConfig never carries — auto-selecting it aborted the run."""
        from phasesweep.validation.calibration import _auto_models
        meas = RunResult(
            config=RunConfig(motor=outrunner_motor, model="backemf_capture",
                             dataset_id="d"),
    ...
    def test_model_valueerror_during_fit_is_rejection(
            self, outrunner_motor, outrunner_backemf, monkeypatch):
...
class TestWriteMotorToml:
    ...
    def test_refuses_overwrite(self, outrunner_motor, tmp_path):
        out = tmp_path / "cal.toml"
        write_motor_toml(outrunner_motor, out)
        with pytest.raises(FileExistsError):
            write_motor_toml(outrunner_motor, out)

    def test_header_written_as_comments(self, outrunner_motor, tmp_path):
        out = tmp_path / "cal.toml"
        write_motor_toml(outrunner_motor, out, header="line one\nline two")
        text = out.read_text()
        assert text.startswith("# line one\n# line two\n")

    def test_escapes_special_chars_in_strings(self, outrunner_motor, tmp_path):
        motor = replace(outrunner_motor, name='cal "v2" \\ backslash')
        out = tmp_path / "esc.toml"
        write_motor_toml(motor, out)
        assert load_motor(out).name == motor.name

    def test_record_sidecar_serializes(self, outrunner_motor, outrunner_backemf, tmp_path):
        result = calibrate(
            outrunner_motor, [outrunner_backemf],
            params=["B_rem"], quantities=["backemf_fundamental"],
        )
        ...
        assert raw["source_config_id"] == outrunner_motor.config_id
...
class TestCliCalibrate:
    ...
    def _run(self, monkeypatch, argv):
        from phasesweep.validation import cli_calibrate
        monkeypatch.setattr(sys, "argv", argv)
        cli_calibrate.main()

    def test_happy_path(self, monkeypatch, capsys, tmp_path, backemf_json):
        out = tmp_path / "outrunner_calibrated.toml"
        self._run(monkeypatch, [
            "phasesweep-calibrate", str(OUTRUNNER_TOML),
    ...
    def test_residual_pairs_align_by_key(self):
        ...
        from phasesweep.validation.cli_calibrate import _residual_pairs
