...
class TestCreatorFixture:
    ...
    def test_fixture_loads(self):
        ...
        assert data.quantities["backemf_fundamental"] == pytest.approx(47.38, abs=0.01)
...
class TestCommittedDatasets:
    ...
    def test_all_import_format_datasets_validate(self):
        ...
        expected = set(_EMF_CONSTANT_DATASETS) | {
            "data/outrunner_14mm_steel/backemf_measured.json"}
    ...
    def test_outrunner_dataset_untagged(self):
        """B_rem in the outrunner TOML is the nominal grade value — fitting
        it against the sweep is the flagship calibration, not an echo."""
        raw = json.loads((
            REPO_ROOT / "data/outrunner_14mm_steel/backemf_measured.json"
        ).read_text())
